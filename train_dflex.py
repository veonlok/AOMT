...
def row_to_arrays(
    row: dict, tok: WordTokenizer, max_len: int, drop_leaky_think: bool = True
) -> Optional[Example]:
    leaky = set()
    if drop_leaky_think:
    ...
    sl = slice(0, max_len)
    return Example(
        np.array(ids[sl], np.int64),
        np.array(btype[sl], np.int64),
        np.array(step[sl], np.int64),
        np.array(bidx[sl], np.int64),
...
def sample_mask(
    obj: str, ex: Example, rng: random.Random, target_ratio: float
    ...
    def fallback():
        n = max(1, int(target_ratio * L))
        s = rng.randint(0, max(0, L - n))
        t = np.zeros(L, bool)
        t[s : s + n] = True
        return ~t, t
    ...
    if obj == "next_action":
        ...
        if not ctx.any():
            ctx = (ex.step == t) & (ex.btype != ACTION)
    elif obj == "retrodiction":
        t = rng.choice(steps[1:]) if len(steps) > 1 else steps[0]
        ctx = ex.step >= t
        tgt = (ex.step >= 1) & (ex.step < t)
        if not tgt.any():
            return fallback()
        ...
        if t2 - t1 < 1:
            t2 = min(steps[-1], t1 + 1)
        ctx = (ex.step <= t1) | (ex.step >= t2) | goal
        tgt = ~ctx
        if not tgt.any():
            return fallback()
...
BASE_WEIGHTS = {
    "next_action": 0.15,
    "next_observation": 0.20,
    "future_from_past": 0.10,
    "full_completion": 0.10,
    "inverse_dynamics": 0.15,
    "retrodiction": 0.10,
    "intermediate_imputation": 0.10,
    "observation_denoising": 0.05,
    "goal_conditioned_action": 0.05,
}
WARMUP = {
    "inverse_dynamics": (0.1, 0.4),
    "retrodiction": (0.2, 0.5),
    "intermediate_imputation": (0.2, 0.5),
    "observation_denoising": (0.3, 0.6),
}
...
def corrupt(ex, ctx, tgt, t, nrng, max_len):
    ...
    btype = np.full(max_len, PAD, np.int64)
    valid = np.zeros(max_len, bool)
    ids[:L] = ex.ids[:L]
    btype[:L] = ex.btype[:L]
    valid[:L] = True
    inp = ids.copy()
    tgt_masked = np.zeros(max_len, bool)
    tgt_masked[:L] = tgt & (nrng.random(L) < t)
    if not tgt_masked.any():
        ti = np.where(tgt)[0]
        tgt_masked[int(nrng.choice(ti)) if ti.size else 0] = True
    hidden = np.zeros(max_len, bool)
    hidden[:L] = valid[:L] & ~ctx & ~tgt
...
if HAS_TORCH:
    ...
    class TimeEmbed(nn.Module):
        ...
        def forward(self, t):
            half = self.dim // 2
            fr = torch.exp(
                -math.log(10000) * torch.arange(half, device=t.device) / max(1, half)
            )
            a = t[:, None] * fr[None, :] * 1000.0
            e = torch.cat([torch.sin(a), torch.cos(a)], -1)
            if e.shape[-1] < self.dim:
                e = F.pad(e, (0, self.dim - e.shape[-1]))
            return self.mlp(e)
...
def make_charts(hist, obj_loss, budget, out_dir):
    ...
    plt.bar(range(len(names)), vals, color=cols)
    ...
    plt.plot(hist["step"], hist["target_ratio"], color="tab:orange")
    plt.xlabel("step")
    plt.ylabel("target ratio r-bar")
    plt.title("Incremental masking curriculum")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{out_dir}/curriculum.png", dpi=130)
    plt.close()
    bt = [TYPE_NAME[k] for k in (GOAL, THINK, ACTION, OBS)]
    bv = [budget.get(k, 0) for k in (GOAL, THINK, ACTION, OBS)]
    plt.figure(figsize=(6, 4))
    plt.bar(bt, bv, color="tab:gray")
    ...
    plt.close()
    return [
        f"{out_dir}/loss_curve.png",
        f"{out_dir}/loss_by_objective.png",
        f"{out_dir}/curriculum.png",
        f"{out_dir}/supervision_budget.png",
    ]
...
def main():
    ...
    tok = WordTokenizer.build(texts, args.max_vocab)
    ...
    model = DFlexDenoiser(
        tok.vocab_size, args.d_model, args.nhead, args.layers, max_len=args.max_len
    ).to(device)
    ...
    def run_batch(exs, obj, progress):
        inp, bt, val_, sc, lab, ts, tr = make_batch(
            exs, obj, progress, rng, nrng, args.max_len
        )
        inp = torch.from_numpy(inp).to(device)
        bt = torch.from_numpy(bt).to(device)
        valid = torch.from_numpy(val_).to(device)
        sc = torch.from_numpy(sc).to(device)
    ...
    for step in range(args.steps):
        ...
        if wb:
            wb.log(
                {
                    "train/loss": lv,
                    "train/objective_idx": ALL_OBJS.index(obj),
        ...
        if step % args.val_every == 0 or step == args.steps - 1:
            ...
            with torch.no_grad():
                ...
                for _ in range(3):
                    vexs = [
                        val[rng.randrange(len(val))] for _ in range(args.batch_size)
                    ]
            ...
            if wb:
                wb.log({"val/loss": vm}, step=step)
            print(
                f"[step {step:4d}/{args.steps}] train={lv:.3f} val={vm:.3f} "
    ...
    torch.save(
        {
            ...
            "config": {
                k: getattr(args, k) for k in ("d_model", "layers", "nhead", "max_len")
            },
    ...
    metrics = {
        ...
        "objective_avg_loss": {
            k: obj_loss[k][0] / obj_loss[k][1] for k in ALL_OBJS if obj_loss[k][1] > 0
        },
    ...
    if wb:
        ...
        wb.log({os.path.basename(c)[:-4]: wandb.Image(c) for c in charts})
        wb.finish()
