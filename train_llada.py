...
def find_lora_targets(model, explicit=None):
    ...
    excl = lambda n: any(
        k in n.lower()
        for k in ["embed", "lm_head", "router", "gate", "expert", "shared"]
    )
...
def make_charts(hist, obj_loss, budget, out_dir):
    ...
    plt.title("LLaDA2.0-mini + D-Flex (LoRA) training")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def main():
    ...
    def loss_on(exs, obj, progress):
        ...
        attn = torch.from_numpy(val_).to(device)
    ...
    for step in range(args.steps):
        ...
        if step % args.val_every == 0 or step == args.steps - 1:
            ...
            print(
                f"[step {step:3d}/{args.steps}] train={acc:.3f} val={vm:.3f} obj={obj} "
