...
def make_charts(hist, obj_loss, budget, out_dir):
    ...
    if hist["val_step"]:
        plt.plot(hist["val_step"], hist["val_loss"], "o-", color="tab:red", label="val (training task)")
        nextobs = hist.get("val_nextobs", [])
        if nextobs and any(not np.isnan(v) for v in nextobs):
            plt.plot(hist["val_step"], nextobs, "s--", color="tab:green",
...
def verify(args, tok, train, val):
    ...
    print(f"  non-scored valid = ctx({ctx_count}) + hidden({hidden_count}), unexpected={int(unexpected.sum())}  (unexpected must be 0)")
    first = np.where(val_)[0][:60]
    shown = ["[MASK]" if sc[p] else tok.decode([int(inp[p])]) for p in first]
    print("-" * 70); print("first ~60 tokens:"); print(" ".join(shown))
    ...
    if args.objective_set == "d_flex":
        ...
        for ex in train[:20]:
            ...
            action_steps = [s for s in _steps_present(ex) if _first_idx(ex, s, ACTION) is not None]
            if action_steps:
                ...
                for t_step in tgt_steps:
                    ...
                    if think_at_t.size:
                        hidden_ok = not ctx[think_at_t].any() and not tgt[think_at_t].any()
                        print(f"  step {t_step}: think hidden (not ctx, not tgt)? {hidden_ok}  (must be True)")
                ...
                if tgt_steps:
                    ...
                    if future_obs.size:
                        ...
                        print(f"  future obs in context (bidirectional)? {fut_in_ctx}  (must be True)")
                        n_ok += 1
                        if n_ok >= 3:
                            break
        if n_ok == 0:
            print("  (no trajectories had future obs to check — OK if all actions are last step)")
    ...
    for ex in train[:5]:
        ...
        if result is None:
            print(f"  {ex.traj_id}: returned None"); continue
        ctx, tgt = result
        ...
        print(f"  {ex.traj_id}: tgt={int(tgt.sum())} obs-tokens  "
...
def main():
    ...
    rng = random.Random(args.seed); nrng = np.random.default_rng(args.seed)
    hist = {"step": [], "loss": [], "mask_rate": [],
            "val_step": [], "val_loss": [], "val_nextobs": []}
    ...
    for step in range(args.steps):
        ...
        if wb:
            wb.log({"train/loss": accloss, "train/mask_rate": achieved,
